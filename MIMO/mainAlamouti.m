...
simulation_name = 'Alamouti_'+string(N)+string(L);
...
n_frame_max = 1000;% Nombre de bits max � simuler
...
S = [mod_psk([0;1]),mod_psk([1;0]);mod_psk([1;0]),mod_psk([0;1])];
...
for i_snr = 1:length(SNR)
    ...
    while (n_frame < n_frame_max)
        ...
        s   = randi([0,1], L*(m/2), 1);      % Message binaire
        x   = step(mod_psk, s);                % Modulation QPSK

        X   = [x(1) , -conj(x(2)); x(2) , conj(x(1))];
        ...
        %% Recepteur
         ...
         [~,I] = min(abs( ( H(:,2)'*Y(:,1) - Y(:,2)'*H(:,1) ) - Hz ).^2,[],2);
        x2 = A(I);
        
        [~,I] = min(abs( ( H(:,1)'*Y(:,1) + Y(:,2)'*H(:,2) ) - Hz ).^2,[],2);
        x1 = A(I);
        
        X_est  =[x1,-conj(x2);x2,conj(x1)];
        
        Lc      = step(demod_psk, X_est(1,:).');         % Démodulation QPSK
        s_est   = double(Lc(1:L*(m/2)) < 0);          % Décision
